...
export default function Rozsak() {
    ...
    if (!termekData) {
        ...
        const filteredTermekData = termekData.filter(elem => elem.plant_category === 106);

        return (
            <div className='row'>
                <Kereses />
                <div className="webshop row mx-auto d-flex align-items-center justify-content-center  col-md-8 col-sm-10">
                    <h1 className='text-center'>Rózsáink</h1>
                    {filteredTermekData.map((elem) => (
                        <TermekKartya
                            adatok={filteredTermekData}
